# Contributing to Designing a Fair Economy

Contributors are welcome to review the reasoning, challenge claims, supply evidence, and propose changes. You do not need to write code or hold formal credentials. Lived experience, counterexamples, and practical implementation concerns are useful when their scope is clear.

Start with the [purpose and overview](README.md), [complete plan](docs/PROJECT_PLAN.md), and [governance process](GOVERNANCE.md). Explain how your contribution affects the relationship between security, practical freedom, diverse outcomes, ownership, power, and a survivable transition. These goals are the current framework; they can also be revised through contributor voting.

## Start reviewing

1. Choose a section or claim. Check existing issues for related work.
2. Open a [review or evidence issue](https://github.com/imjuststeve/designing-a-fair-economy/issues/new?template=review.md). Identify the exact passage, explain the concern, and suggest an improvement. Distinguish evidence, personal experience, inference, and values.
3. For a new mechanism or substantive change, open a [proposal](https://github.com/imjuststeve/designing-a-fair-economy/issues/new?template=proposal.md). Include the exact change, its purpose, alternatives, affected people, costs, power implications, evidence, and uncertainties. Incomplete ideas are welcome as drafts.
4. Comment on existing proposals using the review criteria below. Formal voting opens only after a specific version is ready for review.
5. If comfortable editing files, fork the repository and open a pull request linked to the issue. Otherwise, a maintainer can prepare the edit while crediting your contribution.

High-value starting tasks include checking R05's claim support, recovering blocked sources, examining the transition's effect on households with little financial margin, testing new concentrations of power, and assessing whether education-for-service adds capacity without restricting practical freedom.

## Evaluate a contribution

Address these questions individually; popularity does not answer them:

- Purpose: what human outcome does this advance, and does it preserve the complete framework?
- Evidence: what supports the claim, what contradicts it, and where does it apply?
- Feasibility: who pays, who does the work, what is displaced, and what assumptions matter?
- Distribution: who benefits or bears costs, especially those with little margin?
- Freedom and power: what choices, exit routes, authority, and capture risks change?
- Alternatives and correction: what other approaches exist, what would show failure, and how could it be revised or stopped?

Mark each area supported, uncertain, concern, or not applicable, with reasons. Do not collapse these judgments into a single numerical score. Label unresolved factual claims accurately rather than allowing a vote to establish them as facts.

## Voting and decisions

Contributors decide adoption by vote. Every decision, including the goals and governance rules, can be revised by a later valid vote. The founder has one vote on the same terms as other contributors and no unilateral veto.

A traceable contribution qualifies you: proposals, reviews, explained evidence, relevant lived experience, analysis, editing, and accessibility work can count. Your work need not be accepted or agree with the current proposal. Link to a contribution made before the vote opened. A star, reaction, or ballot alone does not establish eligibility. See [the complete rules](GOVERNANCE.md) for the adopted contribution cutoff and procedures for counting and disputes.

Ideas and evidence can enter discussion without becoming adopted positions; faithful editorial corrections use maintainer review. Substantive additions, revisions, and removals require at least 60 percent approval. Governance, voting rights, and central-purpose changes require at least two-thirds. Approval excludes abstentions; abstentions count toward participation. Once three contributors are eligible, quorum is at least half of the eligible roster, rounded up, with a minimum of three. Below three eligible contributors, ordinary decisions use a strict majority with at least one participant; the two-thirds governance threshold still applies. Missing quorum or no expressed preferences means no decision.

Changes are classified by impact, not by whether they are new or edits. Review and voting each last seven days for ordinary changes and fourteen days for governance or purpose changes. The voting notice freezes the exact wording, eligible roster, tier, quorum, and rules. Challenge omissions or classification during review; material proposal changes require renewed review and a fresh vote.

Adoption and publication are distinct stages. Maintainers must faithfully integrate an adopted change through a linked reviewed pull request; they do not make a second discretionary policy decision. Record the proposal, vote, dissent, final diff check, and published commit. Reactions are not ballots and votes do not establish factual truth.

## Submit work you can share

Cite source URLs and passages, preserve reference IDs, and explain scope limits. Do not upload full copyrighted articles, private data, or material you lack permission to publish. Identify AI assistance and verify the factual claims it helped produce. Humans remain responsible for submissions and ballots.

No general project license has been selected. Reviews can begin now. Before merging substantial contributed text, the maintainer must record the contributor's explicit permission to include that text in this public repository with attribution, or applicable license terms. This does not settle the project's future reuse license or transfer copyright. A broader contribution and licensing policy remains a contributor-governance decision; a vote cannot grant publication rights held by someone else.

## Conduct and review expectations

Critique claims and proposals rather than attacking people. Disagreement and minority views are welcome. Harassment, threats, impersonation, spam, and deliberate ballot manipulation may be moderated. Disclose relevant financial, organizational, or authorship interests without posting private personal information.

Maintainers should explain status changes and exclusions publicly, except where doing so would expose private information or repeat abusive material. Request reconsideration by linking the decision and explaining the revision, whether based on new evidence, a process concern, or a reasoned change of judgment. Response times are targets, not guarantees; no response means pending, not accepted.
